...
def get_endstop_function(name, dim_in, dim_out, stride=[1, 1, 1], dilation=[1, 1, 1], groups=1):
    ...
    endstop_funcs = {
        ...
        "EndStoppingDilationRelu": EndStoppingDilationRelu,
        "EndStoppingDilationConv": EndStoppingDilationConv,
...
class EndstoppingDilationPReLU(nn.Conv3d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=(1, 3, 3), stride=1, padding=(0, 1, 1), dilation=1,
                 bias=True, groups=1):
...
class EndStoppingDilationRelu(nn.Conv3d):
...
class EndStoppingDilationConv(nn.Conv3d):
    ...
    def forward(self, x):
        surround = self.get_surround(self.param2)
        x1 = F.conv3d(x, self.param1, stride=self.stride, dilation=1, padding=self.padding, groups=self.groups)
        x2 = F.conv3d(x, surround, stride=self.stride, dilation=2, padding=(0, 2, 2), groups=self.groups)
        x = x1 + x2
        return x
