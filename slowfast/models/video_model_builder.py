...
from . import head_helper, resnet_helper, stem_helper, endstop_helper
...
class FuseFastAndSlow(nn.Module):
    """
    Fuses the information between the Fast pathway nd the Slow pathway. Given the
    tensors from Slow pathway and Fast pathway, fuse information between Fast and
    Slow, then return the fused tensors from Slow and Fast pathway in order.
    """
    ...
    def __init__(
        ...
        self.conv_f2s = nn.Conv3d(
            ...
            kernel_size=[fast_fusion_kernel, 1, 1],
            stride=[alpha, 1, 1],
            padding=[fast_fusion_kernel // 2, 0, 0],
            bias=False,
        )
        self.s_bn = norm_module(
            num_features=fast_dim_in * fast_fusion_conv_channel_ratio,
            eps=eps,
            momentum=bn_mmt,
        )
        self.s_relu = nn.ReLU(inplace_relu)
        self.conv_s2f = nn.Conv3d(
            slow_dim_in,
            slow_dim_in // slow_fusion_conv_channel_ratio,
            kernel_size=[1, 1, 1],
            stride=[1, 1, 1],
            padding=[0, 0, 0],
            bias=False,
        ...
        self.f_relu = nn.ReLU(inplace_relu)
...
class SlowFast(nn.Module):
    ...
    def _construct_network(self, cfg):
        ...
        self.s1_fuse = FuseFastToSlow(
            ...
            cfg.SLOWFAST.FUSION_TRANS_FUNC,
            cfg.SLOWFAST.ALPHA,
            norm_module=self.norm_module,
        )
...
class SlowFast2(nn.Module):
    """
    SlowFast2 model builder for SlowFast network.
