...
def get_trans_func(name):
    ...
    trans_funcs = {
        ...
        "bottleneck_transform2": BottleneckTransform2,
...
class BottleneckTransform2(nn.Module):
    """
    Bottleneck transformation: Tx1x1, 1x3x3, 1x1x1, where T is the size of
        temporal kernel.
    """
    ...
    def _construct(
        ...
        self.b_bn2 = norm_module(
            num_features=dim_inner, eps=self._eps, momentum=self._bn_mmt
        )
        self.b_relu2 = nn.ReLU(inplace=self._inplace_relu)
    ...
    def _construct(
        ...
        for pathway in range(self.num_pathways):
            for i in range(self.num_blocks[pathway]):
                ...
                # Every Block
                if (trans_func_name == "endstop_bottleneck_transform3x3_v2") and pathway == 1:
                    trans_func = get_trans_func(trans_func_name)
                    res_block = ResBlock(
                        dim_in[pathway] if i == 0 else dim_out[pathway],
                        dim_out[pathway],
